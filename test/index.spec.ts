import themeable, { ClassNameDecorator, StyleThemeDefinition } from '../src';
...
describe('styles with classname decorator (e.g. Aphrodite)', () => {
  ...
  const colorsToString: ClassNameDecorator<CSSProperties> = (...styles) => styles.map(x => x.color).join(' ');
...
describe('style', () => {
  const styles: StyleThemeDefinition<'foo' | 'bar'> = {
    foo: {
      color: 'red',
      fontSize: '16px',
    },
